...
const writeDataArgsSchema = z.object({
  org: z
    .string()
    .describe(
...
const createBucketArgsSchema = z.object({
  name: z
    .string()
    .describe(
...
const createOrgArgsSchema = z.object({
  name: z
    .string()
    .describe(
...
const toMcpInputSchema = (schema: z.AnyZodObject): ZodRawShapeCompat => {
  // The SDK consumes raw Zod shapes; keep object schemas as the typed source of truth.
...
const createMcpServer = () => {
  ...
  server.registerTool<ZodRawShapeCompat, ZodRawShapeCompat>(
    "write-data",
    {
      ...
      inputSchema: toMcpInputSchema(writeDataArgsSchema),
    },
    (args) => writeData(writeDataArgsSchema.parse(args)),
  );
  server.registerTool<ZodRawShapeCompat, ZodRawShapeCompat>(
    "query-data",
    {
      ...
      inputSchema: toMcpInputSchema(queryDataArgsSchema),
    },
    (args) => queryData(queryDataArgsSchema.parse(args)),
  );
  server.registerTool<ZodRawShapeCompat, ZodRawShapeCompat>(
    "create-bucket",
    {
      description: "Provision a new bucket under an organization so that subsequent write-data calls have a destination.",
      inputSchema: toMcpInputSchema(createBucketArgsSchema),
    },
    (args) => createBucket(createBucketArgsSchema.parse(args)),
  );
  server.registerTool<ZodRawShapeCompat, ZodRawShapeCompat>(
    "create-org",
    {
      description: "Create a brand-new organization to isolate users or projects before generating buckets and tokens.",
      inputSchema: toMcpInputSchema(createOrgArgsSchema),
    },
    (args) => createOrg(createOrgArgsSchema.parse(args)),
  );
...
if (!useHttpTransport) {
  ...
  if (isTestMode) {
    ...
    globalServer.connect = async function (transportInstance: Transport) {
      try {
  ...
  const connectStdioServer = async () => {
    try {
      ...
      if (isTestMode) {
        ...
        globalServer.server.onerror = (err: Error) => {
